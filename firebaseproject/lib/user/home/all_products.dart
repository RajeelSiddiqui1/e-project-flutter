...
class _AllProductsScreenState extends State<AllProductsScreen> {
    ...
    if (mounted) {
      setState(() {
        _categories = snapshot.docs
            .map((doc) => {'id': doc.id, 'title': doc['title'] ?? 'Unknown'})
            .toList();
      });
    }
  }
  ...
  Future<void> _toggleWishlist(String productId, Map<String, dynamic> productData, bool isWishlisted) async {
    ...
    final wishlistRef = _firestore.collection('wishlist').doc(user!.uid).collection('items').doc(productId);
    if (isWishlisted) {
  ...
  void _applyFilters() {
    ...
    if (_searchText.isNotEmpty) {
      ...
      filtered = filtered.where((doc) {
        ...
        return name.contains(searchLower) || author.contains(searchLower) || price.contains(searchLower) || category.contains(searchLower);
      }).toList();
    }
    filtered.sort((a, b) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text('Book Heaven', style: TextStyle(fontFamily: 'Georgia', fontSize: 22, fontWeight: FontWeight.bold)),
        centerTitle: true,
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              FilterChip(label: const Text('On Sale Only'), selected: _showDiscountOnly, onSelected: (v) => setState(() => _showDiscountOnly = v)),
              const SizedBox(height: 8),
              FilterChip(label: const Text('Unbeatable Deals (>30% off)'), selected: _showUnbeatableOnly, onSelected: (v) => setState(() => _showUnbeatableOnly = v)),
              const Spacer(),
              SizedBox(width: double.infinity, child: TextButton(onPressed: _clearFilters, child: const Text('Reset Filters'))),
              const SizedBox(height: 8),
              SizedBox(width: double.infinity, child: ElevatedButton(onPressed: () { _applyFilters(); Navigator.pop(context); }, child: const Text('Apply Filters'))),
            ],
          ),
        ),
      ),
...
class _ProductCard extends StatelessWidget {
  ...
  const _ProductCard({required this.productData, required this.productId, required this.isWishlisted, required this.onWishlistToggle});
                ...
                children: [
                  ClipRRect(
                    ...
                    child: imageUrl != null && imageUrl.isNotEmpty
                        ? Image.network(imageUrl, width: double.infinity, fit: BoxFit.cover, loadingBuilder: (c, child, p) => p == null ? child : const Center(child: CircularProgressIndicator(strokeWidth: 2)), errorBuilder: (c, e, s) => const Center(child: Icon(Icons.book_outlined, size: 50)))
                  ...
                  if (discount > 0)
                    Positioned(
                      ...
                      child: Container(
                        ...
                        color: Colors.red,
                        child: Text('${discount.toStringAsFixed(0)}% OFF', style: const TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.bold)),
                      ),
                    ),
                  Positioned(
                    top: 8,
                    right: 8,
                    child: IconButton(icon: Icon(isWishlisted ? Icons.favorite : Icons.favorite_border, color: isWishlisted ? Colors.red : null), onPressed: onWishlistToggle),
                  ),
                ],
              ),
            ),
              ...
              child: Padding(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  ...
                  Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    if (discount > 0) Text('\$${price.toStringAsFixed(2)}', style: const TextStyle(fontSize: 12, color: Colors.grey, decoration: TextDecoration.lineThrough)),
